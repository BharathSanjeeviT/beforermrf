					...
					<div className="space-y-8">
						<div>
							...
							<p className="text-muted-foreground">
								Construction Site, {new Date(eodid).toDateString()}
							</p>
						</div>
						<div>
							...
							<div className="mt-4 grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3">
								{attendence.map((item, idx) => (
									<div className="rounded-lg border bg-card p-4" key={idx}>
										<h3 className="text-lg font-medium">{item.r_name.toUpperCase()}</h3>
										<div className="mt-2 text-muted-foreground">
											<p>{item.u_name}</p>
											<p>
													In: {new Date(item.check_in).toLocaleTimeString()} -
													Out: {new Date(item.check_out).toLocaleTimeString()}
											</p>
										</div>
									</div>
								))}
									...
									<div className="rounded-lg border bg-card p-4" key={idx}>
										...
										<div className="mt-2 text-muted-foreground">
											{item.logs.map((log, idx) => {
												return (
													<p key={idx}>
														{new Date(log.receive_time).toLocaleTimeString()}-{" "}
														{log.quant} units{" "}
														{log.sender === sid
															? log.receiver
																? "sent"
